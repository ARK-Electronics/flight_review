...
| Medium | Browser mutations lacked CSRF validation, and log deletion used GET. | Enable Tornado CSRF protection, send tokens from forms/AJAX, and require POST confirmation for deletion. Explicit API credentials remain supported. |
...
| Dependency | The shared footer loaded jQuery 3.1.0, which predates upstream XSS fixes. | A separate PR updates it to jQuery 3.7.1 with a verified integrity hash. No application-specific exploit of this dependency was demonstrated. |
