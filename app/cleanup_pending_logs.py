...
#   account is approved (process_pending_logs_for_user), on login, or when
#   the plot URL is opened. Use --process to force-parse them here.
#
...
#     cd app && python cleanup_pending_logs.py --process   # parse named pending
...
def main():
    parser = argparse.ArgumentParser(
        description='Clean up logs stuck in the Pending state. Lists by '
                    'default; --process parses named pending logs; --delete '
                    'removes anonymous pending logs.')
    parser.add_argument('--delete', action='store_true',
                        help='Delete anonymous pending logs (files and DB rows).')
    parser.add_argument('--process', action='store_true',
                        help='Parse all pending logs (including anonymous).')
    ...
    try:
        ...
        for log_id, uploader, date in named:
            print('  named {} (uploader={}, date={})'.format(log_id, uploader, date))
        for log_id, date in anonymous:
            print('  anonymous {} (date={})'.format(log_id, date))

        if args.process:
            # Import here so list-only runs don't need the full parse stack.
            from tornado_handlers.upload import process_pending_log
            processed = 0
            failed = 0
            for log_id, _uploader, _date in named:
                print('Processing {} ... '.format(log_id), end='', flush=True)
                try:
            ...
            for log_id, _date in anonymous:
                print('Processing anonymous {} ... '.format(log_id), end='', flush=True)
                try:
                    if process_pending_log(log_id):
                        processed += 1
                        print('done')
            ...
            # Refresh pending set after processing before optional delete.
            cur.execute('select Id, Uploader, Date from Logs where Pending = 1')
            rows = cur.fetchall()
            anonymous = [(log_id, date) for log_id, uploader, date in rows if not uploader]
            named = [(log_id, uploader, date) for log_id, uploader, date in rows if uploader]
        ...
        if not args.delete:
            if anonymous and not args.process:
                print('Re-run with --delete to remove anonymous log(s), or '
                      '--process to parse all pending logs.')
            return
        ...
        print('Deleted {}, failed {}, remaining named pending {}.'.format(
